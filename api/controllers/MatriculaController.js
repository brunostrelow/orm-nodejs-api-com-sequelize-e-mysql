...
class MatriculaController {
    ...
    static async atualizaMatricula(req, res) {
        ...
        try {
            await matriculasServices.atualizaRegistros(novaInfos, { id: Number(matriculaId), estudante_id: Number(estudanteId) })
    ...
    static async restauraMatricula(req, res) {
        const { matriculaId } = req.params
        try {
            await matriculasServices.restauraRegistro( { id: Number(matriculaId) })
    ...
    static async pegaMatriculasPorTurma(req, res) {
        ...
        try {
            const todasAsMatriculas = await matriculasServices
                .encontraEContaRegistros( { turma_id: Number(turmaId), status: 'confirmado' }, { limit: 20, order: [['estudante_id', 'DESC']] })
